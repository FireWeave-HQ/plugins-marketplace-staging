...
> Attach the current coding agent's FireWeave standing loop to a repo that is already initialised (harness, beacon, agent-instructions present). Writes only agent standing instructions + hooks — never re-scaffolds the harness, never rotates beacon keys. Use when a teammate clones an initialised repo and opens Claude Code / Cursor / another agent that was not present at initialise time, or when the user asks to "adopt this repo", "install FireWeave for my agent", or invokes `/fireweave:adopt`. `--check` reports the wanted-vs-present diff without writing.
>
> _User-triggered (Cline does not auto-activate workflows). Run with `/fw-adopt.md`._
...
`/fireweave:initialise --reinit` still re-scaffolds beacon/harness — do **not** use it just to add
an agent loop.
...
Run `use_mcp_tool(server_name="rollout-server", tool_name="ensure_auth")` with
...
Then run the tool-manifest check via `use_mcp_tool(server_name="rollout-server", tool_name="list_registered_tools")`.

## Steps

| Step                                   | Action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1 — Initialised gate (both shapes)** | Read `.fireweave/project.json`. **Discriminate on field presence, never on `version`.** _Legacy shape_ (`rolloutReady` **present**): continue **ONLY** when `rolloutReady.initialized === true`. _Pointer shape_ (`rolloutReady` **absent**, identity present): the repo is initialised — `initialized` has no home in a pointer, and requiring it would PARK `adopt` on precisely the repos that migrated. Confirm instead that the pointer carries `projectId` (or `projects{}`). Else PARK: _"this repo is not initialised — run `/fireweave:initialise` first"_ — **write nothing.** **`adopt` never initialises.** In BOTH shapes, confirm `.fireweave/agent-instructions.md` exists; if missing on an initialised repo → PARK toward `/fireweave:initialise --reinit` (genuine drift, not an adopt case) — **write nothing.** **Why PARK here but WARN for a missing build-gate (N4):** every standing loop points at `agent-instructions.md`, so attaching HARD ORDER / hooks that link a missing SoT is not useful; the build-gate is orthogonal to standing instructions and does not justify a beacon-rotating `--reinit`. Note `mcp.mode`, `installedInto[]`, and whether `teamAgents` is **absent** (pre-teamAgents initialisation — Step 5 will seed) or **present**.                                                                                                                                                                                                                                                                                                                    |
| **2 — Choose agents**                  | Detect coding-agent markers (`.claude/`, `.cursor/`, …). Then one `ask_followup_question` (multi-select) for **this** machine. **V1 full-materialize:** `cursor`, `claude` only. Other hosts are link-only experimental (optional second ask). Host detection alone is unreliable. Empty selection → re-ask once; if still empty → PARK (write nothing).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **3 — Diff**                           | For each selected agent, compute wanted-vs-present against the **Write matrix** below. Report what is missing / stale before writing. **Also read-only check shared build-gate:** note whether `.fireweave/hooks/rollout-build-gate.mjs` and `.fireweave/hooks/rollout-build-gate.sh` exist. If either is **missing** → **WARN** (do **not** PARK, do **not** force `--reinit` / beacon rotate) — standing-loop writes still proceed; adopt must not re-copy build-gate from platform source. If the `.sh` exists but is non-executable → note mode-bit repair for Step 4. On `--check` → stop here (write nothing; still report build-gate status).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **5 — Bookkeeping (server-side)**      | `installedInto[]` and `teamAgents` are **repo-scoped server state**. Write them with `use_mcp_tool(server_name="rollout-server", tool_name="update_repo_state")` — one call, `{ fields: { installedInto: [...], teamAgents: [...] } }` — never by hand-editing `project.json`. Both are **set-valued and UNION on merge**, which is exactly what a second machine adopting concurrently needs: send only what this run added and the server folds it in. The write is online-only and fails closed; on `refused` → PARK and report, do not fall back to editing the file. In a legacy-shaped repo the tool also projects the block into `rolloutReady.installedInto[]` / `rolloutReady.teamAgents` itself — that projection is the tool's job, not yours. Append written paths (sorted, deduped). **Union** selected agents (sorted, deduped). **When `teamAgents` is absent**, first **derive** the existing set exactly as `/fireweave:initialise --reinit` rule 2 — agents implied by `installedInto[]` paths, plus disk markers that already carry FireWeave standing-loop artifacts (HARD ORDER / FireWeave rule / intent-gate — **not** bare `.cursor/` or `.claude/` alone) — then union the selection into **that** derived set; never into a bare `[]` (N3 — first adopt must not drop other agents already wired). A **present** array (including `[]`) is authoritative: union into it as-is (post-`--remove` present-`[]` stays intentional). Record per-machine adopt fact in gitignored `.fireweave/local.json` as `adopt: { agents, at, skillVersion }` — do **not** write beacon / env / capability / `mcp.mode` fields. |
| **6 — Verify**                         | `use_mcp_tool(server_name="rollout-server", tool_name="detect_rollout_ready")` + `use_mcp_tool(server_name="rollout-server", tool_name="reconcile")` with `phase: "build"`. Assert each written script is executable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **7 — Reload notice**                  | Name **only** the agents whose reloadable artifacts were written this run (from this run's `installedInto[]` appends): Cursor → Developer → Reload Window; Claude Code → hooks apply on the next session. If Step 3 warned that build-gate files are missing, surface that warning again (standing loops are attached; build-gate restore still needs `/fireweave:initialise --reinit` when the user wants harness repair — not required to finish adopt). If `mcp.mode` is `cursor-plugin` and the adopting agent is **not** Cursor → **notice only** (do not run): non-Cursor hosts may need `fw mcp install` for MCP transport. Standing instructions ≠ MCP wiring. `--reinit` remains available for harness/beacon drift — prefer `adopt` for standing-loop-only gaps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `cursor`                                                           | `.cursor/rules/fireweave-rollout-ready.mdc`; `.cursor/hooks.json` `sessionStart`/`stop` entries; `.cursor/hooks/fireweave-rollout-{session,stop}.sh` (chmod +x); FireWeave skills under `.cursor/skills/` copied **from the installed plugin bundle only** (never from `packages/fw-plugins/` platform source)                      |
...
**Skill copies are COPIES, not links (C27).** `.cursor/skills/<skill>/SKILL.md` is a
...
`safe-rollout-fast`, `cleanup`), not just the one you were thinking about — a
half-refreshed set is a repo where `safe-rollout-fast` reads the seam and `cleanup`
