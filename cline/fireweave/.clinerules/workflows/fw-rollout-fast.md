...
> The no-swap "promote, not wrap" ship path for repos already initialised with FireWeave Rollouts. Detects the rollout-ready manifest + change stamps, verifies the already-generated prod branch (no provider swap), then registers a ramp on the deploy-liveness gate. Use when the user asks to "ship this", "promote the rollout", "ramp the feature", or invokes `/fireweave:safe-rollout-fast` in an initialised repo.
>
> _User-triggered (Cline does not auto-activate workflows). Run with `/fw-rollout-fast.md`._
...
Call `use_mcp_tool(server_name="rollout-server", tool_name="list_registered_tools")` and confirm every tool in
...
Every clarification uses `ask_followup_question` — never raw open-ended prompts.
