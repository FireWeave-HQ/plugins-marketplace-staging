...
nothing).** Call
`use_mcp_tool(server_name="rollout-server", tool_name="ensure_auth")` with
...
`use_mcp_tool(server_name="rollout-server", tool_name="read_lockfile")` (pass the same `{ cwd }`) and branch on
the result.
...
**Gate `GATE-0-RESUME-DECISION`** — `ask_followup_question`:
<question>Diffs from a previous run are in your working tree. What would you like to do?</question>
<options>["Resume the draft rollout","Cancel draft rollout","Withdraw my participation"]</options>
<!-- options also: Offer "Withdraw my participation" ONLY when state.participantId is present AND get_rollout_status shows more than one participant — the server never removes the last participant. -->
...
**Gate `GATE-0-DRAFT-RESUME`** — `ask_followup_question`:
<question>A draft rollout '<name>' from a previous run is open. Continue, cancel it, or withdraw?</question>
<options>["Continue","Cancel draft rollout","Withdraw my participation"]</options>
<!-- options also: Offer "Withdraw my participation" ONLY when state.participantId is present AND get_rollout_status shows more than one participant — the server never removes the last participant. Substitute <name> with the rollout name from get_rollout_status. -->
  ...
  - **Cancel draft rollout** → this is a **Configuration step** — call
    `cancel_rollout` via `guarded_call` (`fireweave-api`,
...
'CancelRolloutResult' }`). On `{ ok: true }`, call
    `use_mcp_tool(server_name="rollout-server", tool_name="clear_lockfile")`; the skill ends here.
  - **Withdraw my participation** → this is a **Configuration step** —
    call `withdraw_participant` via `guarded_call`
    (`fireweave-api`, `withdraw_participant`, args
...
<options>["Yes, update","No, abort and start fresh"]</options>
   ...
   - **Yes, update** → this is a **Configuration step** — call via `guarded_call`: 1. Resolve the underlying tool name and server prefix
     (`fireweave-api`, `update_participant_sha`). 2. Call `use_mcp_tool(server_name="rollout-server", tool_name="guarded_call")` with
...
expectedResponseSchema: 'UpdateParticipantShaResult' }` (`id` +
   ...
   - **No, abort and start fresh** → clearing the local lockfile does NOT
     ...
     call `cancel_rollout` via `guarded_call` (`fireweave-api`,
     `cancel_rollout`, args `{ id: <rolloutId>,
...
'CancelRolloutResult' }`) so no orphaned draft is left open. - Otherwise — tell the user verbatim: _"The rollout `<rolloutId>`
     ...
     Cancel."_
     Then call `use_mcp_tool(server_name="rollout-server", tool_name="clear_lockfile")`; the user can start
     fresh from Step 0.1.
...
<options>["Yes, update the spec","No, keep the registered spec"]</options>
   ...
   - **Yes, update the spec** → this is a **Configuration step** — call via
     ...
     (`fireweave-api`, `update_rollout_spec`). 2. Call `use_mcp_tool(server_name="rollout-server", tool_name="guarded_call")` with
...
`use_mcp_tool(server_name="rollout-server", tool_name="write_lockfile")` so the next interruption resumes
from the right place.** After `finalize_rollout` succeeds at Step 9 and
Step 10's summary is rendered, call `use_mcp_tool(server_name="rollout-server", tool_name="clear_lockfile")`
...
<!-- options also: plus one entry per open rollout (showing name + state + primary_dev) -->
   ...
   1. This is a **Configuration step** — call `add_rollout_participant`
      ...
      (`fireweave-api`, `add_rollout_participant`). 2. Call `use_mcp_tool(server_name="rollout-server", tool_name="guarded_call")` with
...
'AddRolloutParticipantResult' }`. A `null` `commitSha` joins as a
   ...
   2. Call `use_mcp_tool(server_name="rollout-server", tool_name="write_lockfile")` with
...
3. **Capability discovery.** Run
   ...
   provider, present a per-capability missing-config gate (`ask_followup_question`
...
**Gate `GATE-2.5-REGISTER-DRAFT`** — `ask_followup_question`:
<question>Create rollout '<name>' in <environment> now? It starts in 'drafting' and can be cancelled.</question>
<options>["Yes, create draft","No — exit"]</options>
...
1. Call `use_mcp_tool(server_name="rollout-server", tool_name="write_lockfile")` with
...
`use_mcp_tool(server_name="rollout-server", tool_name="write_lockfile")` again with the same shape
...
Run all seven `use_mcp_tool(server_name="rollout-server", tool_name="verify")_*` tools, in order, each with
...
<question>Finalize this rollout?</question>
<options>["Yes, finalize and capture commit","Edit specific section…","Cancel"]</options>
...
<question>Ready to finalize? I'll need a commit SHA. Have you committed and pushed?</question>
...
`use_mcp_tool(server_name="rollout-server", tool_name="write_lockfile")` with `{ lastStep: 'finalize',
...
3. Call `use_mcp_tool(server_name="rollout-server", tool_name="guarded_call")` with
   `{ serverPrefix, toolName, args: { id: <rolloutId>,
...
1. Resolve the underlying tool name and server prefix
   (`fireweave-api`, `finalize_rollout`).
2. Call `use_mcp_tool(server_name="rollout-server", tool_name="guarded_call")` with
   `{ serverPrefix, toolName, args: { id: <rolloutId> },
isConfigurationStep: true,
...
'fireweave-api', toolName: 'record_pr_url', args: { id:
...
**Gate `GATE-10.1-PROMOTE`** — `ask_followup_question`:
<question>Rollout completed in `<currentEnv>`. Promote to `<nextEnv>`?</question>
<options>["Promote to <nextEnv>","Skip — I'll promote in the portal"]</options>
...
- **Never** write to `.fireweave/project.json` or
  ...
  `use_mcp_tool(server_name="rollout-server", tool_name="write_preferences")` (which also migrates the
