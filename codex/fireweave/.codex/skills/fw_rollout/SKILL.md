...
can ramp them safely. **Follow the steps in order. Use `request_user_input`
...
nothing).** Call
`mcp__rollout_server__ensure_auth` with
...
`mcp__rollout_server__read_lockfile` (pass the same `{ cwd }`) and branch on
the result.
...
**Gate `GATE-0-RESUME-DECISION`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- Options:
  ...
  - **Revert and start over** → run `git restore` on the files listed in
    ...
    `mcp__rollout_server__clear_lockfile`; restart from Step 0.1.
...
**Gate `GATE-0-DRAFT-RESUME`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- Options:
  ...
  - **Cancel draft rollout** → this is a **Configuration step** — call
    `cancel_rollout` via `guarded_call` (`mcp__fireweave_api__`,
...
'CancelRolloutResult' }`). On `{ ok: true }`, call
    `mcp__rollout_server__clear_lockfile`; the skill ends here.
  - **Withdraw my participation** → this is a **Configuration step** —
    call `withdraw_participant` via `guarded_call`
    (`mcp__fireweave_api__`, `withdraw_participant`, args
...
**Gate `GATE-0-FORCE-PUSH-DECISION`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- Options:
  ...
  - No, abort and start fresh
   ...
   - **Yes, update** → this is a **Configuration step** — call via `guarded_call`: 1. Resolve the underlying tool name and server prefix
     (`mcp__fireweave_api__`, `update_participant_sha`). 2. Call `mcp__rollout_server__guarded_call` with
...
expectedResponseSchema: 'UpdateParticipantShaResult' }` (`id` +
   ...
   - **No, abort and start fresh** → clearing the local lockfile does NOT
     ...
     call `cancel_rollout` via `guarded_call` (`mcp__fireweave_api__`,
     `cancel_rollout`, args `{ id: <rolloutId>,
...
'CancelRolloutResult' }`) so no orphaned draft is left open. - Otherwise — tell the user verbatim: _"The rollout `<rolloutId>`
     ...
     Cancel."_
     Then call `mcp__rollout_server__clear_lockfile`; the user can start
     fresh from Step 0.1.
...
3. If `state === 'wrapping'`, we diff the current worktree against
   ...
   > non-empty.
   >
   > 1. Call `request_user_input` with the delta-panel summary and the options
   >    below.
   > 2. Call `mcp__rollout_server__write_confirmation_receipt` with
...
**Gate `GATE-0-SPEC-DELTA-DECISION`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- Options:
  ...
  - No, keep the registered spec
   ...
   - **Yes, update the spec** → this is a **Configuration step** — call via
     ...
     (`mcp__fireweave_api__`, `update_rollout_spec`). 2. Call `mcp__rollout_server__guarded_call` with
...
**Gate `GATE-0-SEALED-RERUN-DECISION`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
- Q: This rollout is `<state>` and its spec is frozen. What would you like to do?
- Options:
  - Start a new rollout

   Then act on the selected option:
   - **Start a new rollout** → call `mcp__rollout_server__clear_lockfile`;
...
`mcp__rollout_server__write_lockfile` so the next interruption resumes
from the right place.** After `finalize_rollout` succeeds at Step 9 and
Step 10's summary is rendered, call `mcp__rollout_server__clear_lockfile`
...
`mcp__rollout_server__<name>` — they are stable local tools, not cloud
forwarders.
...
Call `mcp__rollout_server__list_registered_tools`.
...
**Gate `GATE-0.2-JOIN-OR-CREATE`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- Options:
  ...
  - _plus one entry per open rollout (showing name + state + primary_dev)_
   ...
   1. This is a **Configuration step** — call `add_rollout_participant`
      ...
      (`mcp__fireweave_api__`, `add_rollout_participant`). 2. Call `mcp__rollout_server__guarded_call` with
...
'AddRolloutParticipantResult' }`. A `null` `commitSha` joins as a
   ...
   2. Call `mcp__rollout_server__write_lockfile` with
...
**Gate `GATE-0.2-MULTI-REPO`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
3. **Capability discovery.** Run
   ...
   > 2. Call `request_user_input` with the question and options below.
   > 3. Call `mcp__rollout_server__write_confirmation_receipt` with
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-FEATURE-FLAG-CONTROL`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-OBSERVABILITY-QUERY-METRICS`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-OBSERVABILITY-QUERY-LOGS`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-OBSERVABILITY-QUERY-TRACES`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-CICD-COMMIT-WAS-DEPLOYED`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
7. **Lockfile checkpoint.** Call `mcp__rollout_server__write_lockfile`
...
**Gate `GATE-1-FEATURE-SURFACE`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
2. Call `mcp__rollout_server__extract_diff_surface` with
...
**Gate `GATE-1-FIRST-TIME-DIRS`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
Three sub-prompts, one `request_user_input` each:
...
**Gate `GATE-2-TYPE`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-2.5-REGISTER-DRAFT`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
- **Yes, create draft** → this is a **Configuration step** — call
  ...
  1.  Resolve the underlying tool name and server prefix
      (`mcp__fireweave_api__`, `register_rollout`).
  2.  Call `mcp__rollout_server__guarded_call` with
...
1. Call `mcp__rollout_server__write_lockfile` with
...
**Gate `GATE-3-ROLLOUT-STYLE`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
`request_user_input` with each candidate provider. If none and Fireweave
...
Call `mcp__rollout_server__analyze_codebase` with
...
`request_user_input` with multi-select options for each candidate (default to
...
**Gate `GATE-5-COHORT-KEY`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-5-COHERENCE-GROUPING`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-6-ACCEPT-METRIC`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
Before applying any diffs, call `mcp__rollout_server__write_lockfile` with
...
**Gate `GATE-7-CODEGEN-REVIEW`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
`mcp__rollout_server__write_lockfile` again with the same shape
...
1. Resolve the underlying tool name and server prefix
   (`mcp__rollout_server__`, `write_preferences`).
2. Call `mcp__rollout_server__guarded_call` with
...
Run all seven `mcp__rollout_server__verify_*` tools, in order, each with
...
**Gate `GATE-8-VERIFY-OVERRIDE`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
**Gate `GATE-8.5-REGISTER-OR-EDIT`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
Call `mcp__rollout_server__write_lockfile` with `{ lastStep: 'summary',
...
**Gate `GATE-9-SHA-READY`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
`mcp__rollout_server__write_lockfile` with `{ lastStep: 'finalize',
...
1. Resolve the underlying tool name and server prefix
   (`mcp__fireweave_api__`, `update_participant_sha`).
2. Call `mcp__rollout_server__guarded_call` with
...
2. Resolve the underlying tool name and server prefix
   (`mcp__fireweave_api__`, `update_rollout_spec`).
3. Call `mcp__rollout_server__guarded_call` with
   `{ serverPrefix, toolName, args: { id: <rolloutId>,
...
1. Resolve the underlying tool name and server prefix
   (`mcp__fireweave_api__`, `finalize_rollout`).
2. Call `mcp__rollout_server__guarded_call` with
   `{ serverPrefix, toolName, args: { id: <rolloutId> },
isConfigurationStep: true,
...
1. Resolve the underlying tool name and server prefix
   (`mcp__rollout_server__`, `tag_baseline_commit`).
2. Call `mcp__rollout_server__guarded_call` with
...
`mcp__rollout_server__clear_lockfile` — the work is checkpointed in the
...
**Gate `GATE-9-COMMIT-AND-PR`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
1. Compose the commit message using the rollout's primary flag and
   ...
   follow-up `request_user_input` for edit; the default is the template
   verbatim.
...
'mcp__fireweave_api__', toolName: 'record_pr_url', args: { id:
...
- **Never** write to `.fireweave/project.json` or
  ...
  `mcp__rollout_server__write_preferences` (which also migrates the
