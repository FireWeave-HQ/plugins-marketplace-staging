# Fireweave MCP servers — one-time setup (codex)

This plugin bundles 1 MCP server(s):

- `rollout-server`

On **codex** the server launch path must be written into the host's config with
...
```sh
fw mcp install codex --from /path/to/this/bundle
```

(`--from` defaults to the current directory if you run it from inside the bundle.)
It resolves the absolute `launcher.sh` path and merges it into `~/.codex/config.toml` (`[mcp_servers.*]`),
