...
| ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `.fireweave/project.json` — the **pointer**: `orgId`, `projectId` (or a `projects{}` map), `server.url` | Rollout-ready manifests → `repo_manifests`, authored via `rollout-server_upsert_rollout_manifest`       |
...
| `.fireweave/PROVIDERS.md`                                                                               | Repo-scoped config → `repo_state`, written via `rollout-server_update_repo_state`                       |
...
branch that would false-green `rollout-server_verify_prod_path` (which skips
...
**0a — Profile.** Run `rollout-server_ensure_auth` with
...
- Create denied or indeterminate-while-enforced → use
  ...
  `select_project`.
- `bound: false` / incomplete bind → `rollout-server_select_project`
...
`verify_prod_path`, `assert_dev_checklist` and the beacon would all start failing
for a repo that is otherwise healthy. Keys leave this table when they get server
homes, not when an agent deletes them.
...
2. Declare the whole list to the server in ONE call —
   ...
   second row attesting alongside the first forever.
...
5. Regenerate each harness `FW_SURFACES` block (Step 4) so its runtime
   `surfaceId` **equals** the id recorded for that surface in `surfaces[]` — the id
   the beacon attests at runtime must match the recorded `surfaces[].surfaceId`,
   and both must be an id step 2's declaration returned.
...
After writing `surfaces[]`, run `rollout-server_verify_rollout_config_schema`
...
IDs are the server's):** `grep -rn "sfc_REPLACE_ON_INIT" <every scaffolded harness path>`
...
attest time (the SDK FILTERS it out of the beacon, so the surface silently loses
deploy attribution rather than erroring). Fix by re-running step 2's
`fw repo declare-surfaces` and writing what it returns into both `surfaces[]` and
the harness — never by minting an id to make the grep pass.
...
**Tool:** `rollout-server_provision_deploy_beacon_env` — calls `POST /v1/projects/:projectId/deploy-beacon-keys` via `fw api` (CLI bearer token). The session-gated `/api/projects/:id/ingest-keys` route is for the web control plane only.

**After the tool returns:** `the `question` tool` — _"Where does each prod-tier environment run?"_ Offer ONLY the destinations `cloudSecretDestinations` returns for this repo's detected deploy targets (Render dashboard, GitHub Actions secrets, docker-compose, VM/process env, …) — **do not hardcode the option list**; it is derived from Step 2 detection. Paste the matching block from `cloudSecretDestinations`. **Verification is on trust:** no tool reads back a remote secret store, so the skill accepts the user's confirmation; the enforcing gate is the deploy-time attestation failing if the pair is absent. Then **soft-continue (awaiting-user — not PARK)** until the user confirms secrets are set.
...
**Always confirm the source with `the `question` tool` before scaffolding** — do not
...
environments `prod`/`uat`), `the `question` tool` to confirm the mapping and emit it as
...
snake_case names are already correct in the template filenames — see **Python
surface** below). Two files have **no** template and are authored by their own
...
- `sfc_REPLACE_ON_INIT` — the `sfc_` id Step 3f's `fw repo declare-surfaces`
  returned for THIS surface, matched on the surface's `path` (Step 4 /
  `surfaces[]`). The server owns `sfc_` identity; you never mint one.
- the `// fw:env-source` region (`# fw:env-source` in Python) — `readEnvSignal()`
...
`makeConnectedVendorProvider()`, `initFwTelemetry()`, the flag hooks, the
`FW_STAMPS` plain static import, `initFwAttestation()` — is load-bearing and
pinned by tests. Do not re-derive, re-order, or "tidy" it.
...
## Python surface (Step 2 / 4 / 6 — flags prod-capable, beacon deferred)
...
lists it with the `fireweave` vendor), with the **boot beacon still deferred**: there
is no python deploy SDK to send it with, and `verify_prod_path` records that as a
named skip rather than a pass. Flags and deploy attestation are independent axes;
shipping the flag path does not entitle the beacon to be claimed.
...
- `FW_DEFAULT_ENV` from Step 3a and the `# fw:env-source` block from Step 3d (Python
  ...
  plumb. Telemetry stays **console** on both tiers until the python deploy SDK ships —
  a half-wired OTLP exporter looks configured and silently drops every span, which is
  worse than console. **No boot beacon** is emitted (no python deploy SDK);
  `verify_prod_path` records that as a named skip, so do not add one by hand.
...
1. **Locate the identity seam.** Find where the app learns who the user is — the auth
   ...
   `the `question` tool` when it is ambiguous; do not guess.
2. **Assert it is unconditional.** The bind must not sit inside an `if` / `while` / `switch`
   ...
   `rollout-server_assert_dev_checklist` blocks these shapes on every feature — fix
...
reached through `rollout-server_upsert_rollout_manifest` and the resolution
...
3. **BEFORE calling the task done** — run `rollout-server_assert_dev_checklist` with `{ feature }`. **PARK on any block.** Checklist hard-fails if `telemetry.metrics[].name` entries are declared without emit sites in wrap-point files (dummy / registry-only metrics are forbidden). Also run `detect_rollout_ready` + `reconcile` phase `build`.
...
1. **FIRST** author the manifest with `rollout-server_upsert_rollout_manifest` `{ feature, manifest, baseContentHash }` (Manifest contract in agent-instructions) + mint `chg_`/`stmp_` + apply the stamp policy (per-surface stamps by default — append each stamp to its own surface's `FW_STAMPS`; one shared stamp only when single-project + every surface's harness is surface-aware). **FireWeave stores the manifest — do not write a manifest file yourself.**
2. Implement behind the harness OpenFeature provider with `// @fireweave-flag <key>` at each evaluation site.
3. **BEFORE done** call `rollout-server_assert_dev_checklist` with `{ feature }` — PARK on any block (includes dummy metrics with no emit sites). Also `reconcile` phase `build`.
...
1. **FIRST** — author the rollout-ready manifest by calling
   `rollout-server_upsert_rollout_manifest` `{ feature, manifest, baseContentHash }`
...
1. **HARD:** Use the **Cursor FireWeave plugin MCP** (`plugin-fireweave-rollout-server`). Confirm via `rollout-server_list_registered_tools`.
...
| ----------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ts-server` | `@fireweaveai/deploy-sdk @fireweaveai/sdk`     | deploy-sdk carries attest + OTel + the flag-anchor scanner; the standalone SDK carries control points — the remote adapter and the local dev provider the scaffolded providers bind |
...
| `python`    | `fireweave[openfeature]`                       | the whole surface — flags, runtime, target registration. No deploy-sdk: it is a TypeScript package, which is also why the python beacon is deferred.                                |
...
Bun is the default when nothing is detected and the repo is otherwise Node-shaped. **Two or more lockfiles, or none in a repo with no clear shape → `the `question` tool`** rather than guessing; installing with the wrong manager writes a second lockfile, which is a mess the user then has to unpick. Every clarification in this skill goes through `the `question` tool` — this is not an exception.
...
- `rollout-server_update_repo_state` owns the repo-scoped config —
...
- `rollout-server_upsert_rollout_manifest` authors a rollout-ready
