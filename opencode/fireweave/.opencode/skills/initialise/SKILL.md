...
**SDK dependencies:** per surface, bare specifiers. ts-server: `bun add @fireweaveai/deploy-sdk @fireweaveai/sdk` (deploy-sdk carries attest + OTel + the flag-anchor scanner; the standalone SDK carries control points — the remote adapter and the local dev provider the scaffolded providers bind). Web: `bun add @fireweaveai/deploy-sdk @fireweaveai/web-sdk` (web-sdk carries browser control points; deploy-sdk carries credential resolution + telemetry + hooks). **Never write a version range into these commands.** npm's dist-tags are the source of truth for what "current" means, and a literal pinned here freezes every repo the skill scaffolds at whatever line was current the day the sentence was written — a `0.x` caret caps at the next minor, so those repos stop receiving SDK fixes silently and look deliberately pinned. Record the resolved deploy-sdk version in `rolloutReady.deploySdkVersion`; the FIR-359 harness report carries the installed `@fireweaveai/sdk` version.
...
- **Installing staging requires `--exact`:** `bun add --exact @fireweaveai/deploy-sdk@staging`. Without it bun writes a caret on the prerelease (`^0.2.1-staging.1`), and that range admits later **stable** releases (`0.2.1`, `0.2.2`, …), so the repo silently leaves the staging channel on the next stable publish.
...
The standalone SDKs (https://github.com/FireWeave-HQ/fireweave-sdk) are REQUIRED by the scaffolded harness, not optional extras: the ts-server providers bind `@fireweaveai/sdk`'s remote adapter and local dev provider, and the web providers bind `@fireweaveai/web-sdk` — deploy-sdk stays alongside for attest, telemetry, hooks, and credential resolution. Only when the user explicitly opts into SDK co-development (`rolloutReady.sdkDev: true` or `FIREWEAVE_SDK_DEV=1`) use `workspace:*` for these packages instead of the registry.
