...
`rollout-server_detect_rollout_ready` and confirm the anchor count is
...
reinstalled. Then re-run `rollout-server_detect_rollout_ready` and confirm
...
```
rollout-server_record_rollout_env_contract { cwd, apiSurface, webSurface, webappUrl? }
```
...
`rollout-server_update_repo_state`:
...
`.fireweave/project.json`, or call `rollout-server_select_project` if the
