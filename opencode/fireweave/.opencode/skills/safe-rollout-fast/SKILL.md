...
> `assert_dev_checklist` reports it — use `/fireweave:safe-rollout` (the legacy
> wrap-from-scratch path) instead. **Do not test for a manifest DIRECTORY:**
> manifests are server rows (`repo_manifests`), authored through
> `upsert_rollout_manifest`. In a legacy-shaped repo the tool also materialises
> a tracked file as a projection, so the directory's absence proves nothing
> about whether the repo is initialised.
>
...
Call `rollout-server_list_registered_tools` and confirm every tool in
